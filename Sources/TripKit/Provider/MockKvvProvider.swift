...
public class MockKvvProvider: AbstractEfaProvider {
    ...
    public override func queryTrips(from: Location, via: Location?, to: Location, date: Date, departure: Bool, tripOptions: TripOptions, completion: @escaping (HttpRequest, QueryTripsResult) -> Void) -> AsyncRequest {
        ...
        let trip5 = Trip(id: "", from: from, to: to, legs: [PublicLeg(line: createLine(.tram, "4"), destination: createStation("Tivoli"), departureStop: createStop("Kronenplatz (Kaiserstraße)", createTime(9, 46)), arrivalStop: createStop("Europaplatz (Kaiserstraße)", createTime(9, 52)), intermediateStops: createIntermediates(createTime(9, 46)), message: nil, journeyContext: nil)], fares: fares)
