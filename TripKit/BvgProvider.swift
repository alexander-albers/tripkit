...
//    }
    ...
    override func newLine(network: String?, product: Product?, name: String?, shortName: String?, number: String?) -> Line {
        ...
        if product == .suburbanTrain && label == "S41" {
            ...
            return Line(id: nil, network: network, product: product, label: label, name: nil, style: lineStyle(network: network, product: product, label: label), attr: [.circleAnticlockwise], message: nil)
        } else if product == .bus && label == "S41" {
            return Line(id: nil, network: network, product: product, label: label, name: nil, style: lineStyle(network: network, product: product, label: label), attr: [.serviceReplacement, .circleClockwise], message: nil)
        } else if product == .bus && label == "S42" {
            return Line(id: nil, network: network, product: product, label: label, name: nil, style: lineStyle(network: network, product: product, label: label), attr: [.serviceReplacement, .circleAnticlockwise], message: nil)
